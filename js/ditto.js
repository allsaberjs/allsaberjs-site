...
function selectVersions () {
  var versions = '<select class="selectv" id="mySelect" onchange=changeVersion(event) >' +
    '<option>v1.x</option>' +
    '<option>v2.x</option>' +
  '</select>';
  return versions
...
function changeVersion (e) {
  console.warn('option:', e.target.value);
  ...
  $.get(path, function(data) {
    ...
    (function() {
      ...
      (function() {
        ...
        dsqCount.async = true;
        dsqCount.src = 'http://' + window.disqus_shortname + '.disqus.com/count.js';
